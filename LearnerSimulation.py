...
parser.add_argument('--title', type=str, default='Differ-GSpar')
...
def get_upload(g_new, g_old, dev):
    # print("Receiving:", g_new)
    g_differential = []
    for idx, g_layer in enumerate(g_new):
        g_layer_tmp = torch.zeros_like(g_layer).cuda(dev)
        g_layer_tmp = g_layer - g_old[idx]
        g_differential.append(g_layer_tmp)

    upload_gradient = []
    upload_num = 0
    param_num = 0
    prob = greedy(g_differential)
    for idx, g_layer in enumerate(g_differential):
...
def run(workers, models, save_path, train_data_list, test_data, iterations_epoch):
    ...
    trainloss_file = './ErrCmpnGspar' \
        + args.title \
        + '_' + args.method \
        + '_' + args.model \
        + '_lr' + str(args.lr) \
        + '_bsz' + str(args.train_bsz) + '_B' + str(args.byzantine) \
        + '_V' + str(int(args.V)) \
        + '_E' + str(args.loops) \
    ...
    for epoch in range(args.epochs):
        ...
        for i in workers:
            ...
            if global_clock == 1:
                 g_change = [torch.zeros_like(g_layer).cuda(dev)+g_layer for g_layer in delta_ws]
                 g_remain_list.append(g_change)
                ...
                new_g_avg = [torch.zeros_like(g_layer).cuda(dev) + g_layer for g_layer in delta_ws]
                for idx, g_layer in enumerate(delta_ws):
                    new_g_avg[idx] += args.lr * args.alpha * (h_last_list[i - 1][idx] - h_remain_list[i - 1][idx])

                g_change, sparsification_ratio = get_upload(new_g_avg, g_remain_list[i - 1], dev)
                ...
                h_last_list[i - 1] = [torch.zeros_like(g_layer) for g_layer in delta_ws]
                for idx, g_layer in enumerate(delta_ws):
                    h_last_list[i - 1][idx] = h_remain_list[i - 1][idx] * args.beta
                    h_last_list[i - 1][idx] -= (1/args.lr) * (delta_ws[idx] - g_change[idx])
